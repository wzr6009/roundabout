...
class RoundaboutEnv(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "uncertain":True,#显示
            "ego_attention": True,#显示
            "save_speed":True,#保存速度数据
            "save_speed_picture":True,#保存图像数据
            "is_Dsac":False,
            "num":0
        })
        return config
...
##########################
    ...
    def _make_road(self) -> None:
        ...
        w = 2*np.pi/dev
        my_angle_cos0 = math.cos(math.pi/3*0)
        my_angle_sin0 = math.sin(math.pi/3*0)
        my_angle_cos1 = math.cos(math.pi/5*-1)
        my_angle_sin1 = math.sin(math.pi/5*-1)
        my_angle_cos2 = math.cos(math.pi/5*-3)
        my_angle_sin2 = math.sin(math.pi/5*-3)
        my_angle_cos3 = math.cos(math.pi/5*1)
        my_angle_sin3 = math.sin(math.pi/5*1)
        
        
        net.add_lane("ser", "ses", StraightLane([2, access*my_angle_cos0], [2,dev/2*my_angle_cos0], line_types=(s, c)))
        ...
        net.add_lane("sxs", "sxr", StraightLane([-2,dev/2*my_angle_cos0], [-2, access*my_angle_cos0], line_types=(n, c)))

        net.add_lane("eer", "ees", StraightLane([access*my_angle_cos1+2*my_angle_sin1, -2*my_angle_cos1+access*my_angle_sin1], [dev / 2*my_angle_cos1+2*my_angle_sin1, -2*my_angle_cos1+dev/2*my_angle_sin1], line_types=(s, c)))
        net.add_lane("ees", "ee", SineLane([dev / 2*my_angle_cos1+(2+a)*my_angle_sin1, (-2-a)*my_angle_cos1+dev/2*my_angle_sin1], [(dev / 2 - delta_st)*my_angle_cos1+(2+a)*my_angle_sin1, (-2-a)*my_angle_cos1+(dev / 2 - delta_st)*my_angle_sin1], a, w, -np.pi / 2, line_types=(c, c)))
        net.add_lane("ex", "exs", SineLane([(-dev / 2 + delta_en)*my_angle_cos1-(2+a)*my_angle_sin1, (2+a)*my_angle_cos1+(-dev / 2 + delta_en)*my_angle_sin1], [dev / 2*my_angle_cos1+(-2-a)*my_angle_sin1, (2+a)*my_angle_cos1+dev/2*my_angle_sin1], a, w, -np.pi / 2 + w * delta_en, line_types=(c, c)))
        net.add_lane("exs", "exr", StraightLane([dev / 2*my_angle_cos1-2*my_angle_sin1, 2*my_angle_cos1+dev/2*my_angle_sin1], [access*my_angle_cos1-2*my_angle_sin1, 2*my_angle_cos1+access*my_angle_sin1], line_types=(n, c)))



        net.add_lane("fer", "fes", StraightLane([access*my_angle_cos3+2*my_angle_sin3, -2*my_angle_cos3+access*my_angle_sin3], [dev / 2*my_angle_cos3+2*my_angle_sin3, -2*my_angle_cos3+dev/2*my_angle_sin3], line_types=(s, c)))
        net.add_lane("fes", "fe", SineLane([dev / 2*my_angle_cos3+(2+a)*my_angle_sin3, (-2-a)*my_angle_cos3+dev/2*my_angle_sin3], [(dev / 2 - delta_st)*my_angle_cos3+(2+a)*my_angle_sin3, (-2-a)*my_angle_cos3+(dev / 2 - delta_st)*my_angle_sin3], a, w, -np.pi / 2, line_types=(c, c)))
        net.add_lane("fx", "fxs", SineLane([(-dev / 2 + delta_en)*my_angle_cos3-(2+a)*my_angle_sin3, (2+a)*my_angle_cos3+(-dev / 2 + delta_en)*my_angle_sin3], [dev / 2*my_angle_cos3+(-2-a)*my_angle_sin3, (2+a)*my_angle_cos3+dev/2*my_angle_sin3], a, w, -np.pi / 2 + w * delta_en, line_types=(c, c)))
        net.add_lane("fxs", "fxr", StraightLane([dev / 2*my_angle_cos3-2*my_angle_sin3, 2*my_angle_cos3+dev/2*my_angle_sin3], [access*my_angle_cos3-2*my_angle_sin3, 2*my_angle_cos3+access*my_angle_sin3], line_types=(n, c)))


        net.add_lane("ner", "nes", StraightLane([access*my_angle_cos2+2*my_angle_sin2, -2*my_angle_cos2+access*my_angle_sin2], [dev / 2*my_angle_cos2+2*my_angle_sin2, -2*my_angle_cos2+dev/2*my_angle_sin2], line_types=(s, c)))
        net.add_lane("nes", "ne", SineLane([dev / 2*my_angle_cos2+(2+a)*my_angle_sin2, (-2-a)*my_angle_cos2+dev/2*my_angle_sin2], [(dev / 2 - delta_st)*my_angle_cos2+(2+a)*my_angle_sin2, (-2-a)*my_angle_cos2+(dev / 2 - delta_st)*my_angle_sin2], a, w, -np.pi / 2, line_types=(c, c)))
        net.add_lane("nx", "nxs", SineLane([(-dev / 2 + delta_en)*my_angle_cos2-(2+a)*my_angle_sin2, (2+a)*my_angle_cos2+(-dev / 2 + delta_en)*my_angle_sin2], [dev / 2*my_angle_cos2+(-2-a)*my_angle_sin2, (2+a)*my_angle_cos2+dev/2*my_angle_sin2], a, w, -np.pi / 2 + w * delta_en, line_types=(c, c)))
        net.add_lane("nxs", "nxr", StraightLane([dev / 2*my_angle_cos2-2*my_angle_sin2, 2*my_angle_cos2+dev/2*my_angle_sin2], [access*my_angle_cos2-2*my_angle_sin2, 2*my_angle_cos2+access*my_angle_sin2], line_types=(n, c)))

        # net.add_lane("ner", "nes", StraightLane([-2*my_angle_cos2+access*my_angle_sin2, -access*my_angle_cos2-2*my_angle_sin2], [-2*my_angle_cos2+(-dev/2)*my_angle_sin2, -dev / 2*my_angle_cos2-2*my_angle_sin2], line_types=(s, c)))
    ...
    def _make_vehicles(self) -> None:
        ...
        num_entering_vehicle = np.random.randint(3,4)
