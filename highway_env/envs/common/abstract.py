...
from ego_attention import graphics
from robust.graphics.robust_graphics import DiscreteRobustPlannerGraphics, IntervalRobustPlannerGraphics

from my_lib import muti_display
...
class AbstractEnv(gym.Env):
    ...
    def render(self,mode: str = 'human') -> Optional[np.ndarray]:
        ...
        myenv = self
        is_Dsac = self.config['is_Dsac']
        
        if self.config['uncertain'] and self.config['ego_attention']:
            # print('111111111111111111')
            self.viewer.set_agent_display(
                lambda agent_surface, sim_surface: muti_display.Muti_display.display(sim_surface,self.policy,agent_surface,self.obs,is_Dsac,myenv))


        if self.config['uncertain'] and not self.config['ego_attention']:
            
            self.viewer.set_agent_display(
                lambda agent_surface, sim_surface: DiscreteRobustPlannerGraphics.display(sim_surface,myenv))
            

        if self.config['ego_attention'] and not self.config['uncertain'] :
            
            
            self.viewer.set_agent_display(
            lambda ego_agent_surface, sim_surface_ego: graphics.EGOGraphics.display(self.policy,ego_agent_surface,sim_surface_ego,self.obs,is_Dsac,myenv))
        ...
        if mode == 'rgb_array':
            ...
            return image
        # # self.unwrapped.viewer.directory = self.run_directory
        ##########################
        image = self.viewer.get_image()
        if self.config['save_speed_picture'] :
            with open('/home/linux/wzr_program_manager/RL/RL_lib/distributional-sac-master/test_epoch.json', 'r') as f:
                test_epoch = json.load(f)
            # test_epoch = test_epoch['epoch'] 
            
            self.config['num'] = self.config['num']+1
            
            if self.config['num']%10==0 :
                time = datetime.now().strftime("%Y%m%d-%H%M%S")
                plt.imshow(image[0:1200])
                # plt.show()
                plt.xticks([])

                plt.yticks([])
                # plt.gcf().set_size_inches(20, 12) # get current figure
                plt.savefig("./result/"+str(test_epoch)+str(time)+".svg",dpi=800) 
        if self.config['save_speed']:
        
            with open('/home/linux/wzr_program_manager/RL/RL_lib/distributional-sac-master/test_epoch.json', 'r') as f:
                test_epoch = json.load(f)
            
            
            with open('./result/'+str(test_epoch)+'speed', 'a') as fa:
                fa.write(str(self.vehicle.speed))
                fa.write('\n')
        
    # def my_ego_attention(self,policy,obs):
    #     '''obs = state policy = net_work'''
        
    #     is_Dsac = self.config['is_Dsac']
    #     myenv = self
    #     self.viewer.set_agent_display(
    #         lambda ego_agent_surface, sim_surface_ego: graphics.EGOGraphics.display(policy,ego_agent_surface,sim_surface_ego,obs,is_Dsac,myenv))
