...
class RoadNetwork(object):
    ...
    def get_lane(self, index: LaneIndex) -> AbstractLane:
        ...
        #print(index)
        if index == ('sxs', 'sxr', 1) or index == ('sx', 'sxs', 1):
            _id = 0
            
        if index == ('ses', 'ser', 1) or index == ('se', 'ses', 1):
            _id = 0
            

        if index == ('exs', 'exr', 1) or index == ('ex', 'exs', 1):
            _id = 0
           
        if index == ('ees', 'eer', 1) or index == ('ee', 'ees', 1):
            _id = 0
            

        if index == ('nxs', 'nxr', 1) or index == ('nx', 'sns', 1):
            _id = 0
           
        if index == ('nes', 'ner', 1) or index == ('ne', 'nes', 1):
            _id = 0
            

        if index == ('wxs', 'wxr', 1) or index == ('wx', 'wxs', 1):
            _id = 0
            print('跳过')
        if index == ('wes', 'wer', 1) or index == ('we', 'wes', 1):
            _id = 0
            print('跳过')
        #print(self.graph['sxs']['sxr'][1])
        # if index == ('sx', 'sxs', 1):
